...
interface WorkOrder {
  id: string; title: string; asset: string; priority: string;
  status: string; assignee: string; due: string; cost: number;
}
interface Asset {
  id: string; name: string; category: string; location: string;
  status: string; lastService: string; nextService: string; value: number;
}
interface InventoryItem {
  id: string; name: string; category: string; stock: number;
  reorderLevel: number; unit: string; location: string;
  unitCost: number; supplier: string;
}
interface MaterialRequest {
  id: string; requestedBy: string; workOrder: string; item: string;
  qty: number; unit: string; status: string; requestDate: string;
  requiredDate: string; totalCost: number;
}
interface StatChip { label: string; value: string | number; cls: string; }
...
const statusColor: Record<string, string> = {
  Open: "badge-blue", "In Progress": "badge-purple", Completed: "badge-green",
  "On Hold": "badge-gray", Approved: "badge-green", Pending: "badge-yellow",
  Issued: "badge-blue", Rejected: "badge-red", Active: "badge-green",
  "Under Maintenance": "badge-orange", Critical: "badge-red",
  High: "badge-orange", Medium: "badge-yellow", Low: "badge-green",
};

// ── Shared components ─────────────────────────────────────────────────────────

const Badge = ({ label }: { label: string }) => (
  <span className={`badge ${statusColor[label] ?? "badge-gray"}`}>{label}</span>
);

// ── Grid components ───────────────────────────────────────────────────────────

function WorkOrdersGrid({ data, search }: { data: WorkOrder[]; search: string }) {
  const filtered = useMemo(() =>
    data.filter(r => Object.values(r).join(" ").toLowerCase().includes(search.toLowerCase())),
    [data, search]
  );
  return (
...
function AssetsGrid({ data, search }: { data: Asset[]; search: string }) {
  const filtered = useMemo(() =>
    data.filter(r => Object.values(r).join(" ").toLowerCase().includes(search.toLowerCase())),
    [data, search]
  );
  return (
    <table className="grid-table"><thead><tr>
      ...
      {filtered.map((r, i) => <tr key={i}>
        ...
        <td><Badge label={r.status}/></td>
        <td className="muted">{r.lastService}</td>
        <td className="muted">{r.nextService}</td>
...
function InventoryGrid({ data, search }: { data: InventoryItem[]; search: string }) {
  const filtered = useMemo(() =>
    data.filter(r => Object.values(r).join(" ").toLowerCase().includes(search.toLowerCase())),
    [data, search]
  );
  return (
    <table className="grid-table"><thead><tr>
      ...
      {filtered.map((r, i) => <tr key={i}>
        ...
        <td className="muted">{r.reorderLevel}</td>
        <td className="muted">{r.unit}</td>
        <td className="muted">{r.location}</td>
        <td className="num-cell">${r.unitCost.toFixed(2)}</td>
...
function MaterialRequestsGrid({ data, search }: { data: MaterialRequest[]; search: string }) {
  const filtered = useMemo(() =>
    data.filter(r => Object.values(r).join(" ").toLowerCase().includes(search.toLowerCase())),
    [data, search]
  );
  return (
    <table className="grid-table"><thead><tr>
      ...
      {filtered.map((r, i) => <tr key={i}>
        <td className="id-cell">{r.id}</td>
        <td>{r.requestedBy}</td>
        <td className="wo-cell">{r.workOrder}</td>
        <td>{r.item}</td>
        <td className="num-cell">{r.qty}</td>
        <td className="muted">{r.unit}</td>
        <td><Badge label={r.status}/></td>
        <td className="muted">{r.requestDate}</td>
        <td className="muted">{r.requiredDate}</td>
        <td className="num-cell">${r.totalCost.toLocaleString()}</td>
      </tr>)}
    </tbody></table>
  );
}
...
export default function App() {
  ...
  const stats: Record<TabKey, StatChip[]> = {
    workorders: [
